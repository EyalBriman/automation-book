...
def ensure_custom_assets(docs_dir: Path) -> None:
    ...
    media_dir.mkdir(parents=True, exist_ok=True)
    for name in ["1-7-1d-closed-loop.png", "1-1-3-kcl-circuit.png"]:
        src = custom_dir / name
        if src.exists():
            shutil.copy2(src, media_dir / src.name)


def exercise_solution_override(number: str, solution_html: str) -> str:
    """Override individual exercise fragments where Word/Pandoc breaks drawings."""
    if number == "1.1.3":
        soup = BeautifulSoup(solution_html, "html.parser")
        img = soup.find("img")
        if img is not None:
            img["src"] = "media/1-1-3-kcl-circuit.png"
            img["alt"] = "מעגל חשמלי עם סימון הזרמים i1, i2, i3 עבור מציאת מודל דינמי למתח על הקבל"
            img["loading"] = "lazy"
            img["dir"] = "ltr"
            img["style"] = "width:6in; max-width:100%;"
        for tag in list(soup.find_all(["p", "div"])):
            txt = "".join(tag.get_text(" ", strip=True).split())
            if re.fullmatch(r"i[123]", txt):
                tag.decompose()
        return str(soup)
...
def build_data(html_path: Path, temp_dir: Path, docs_dir: Path) -> dict:
    ...
    for spec in EXERCISES:
        ...
        solution_html = exercise_solution_override(spec.number, solution_html)
